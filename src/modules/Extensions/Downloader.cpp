...
void DownloaderThread::run()
{
    ...
    if (isFFmpeg || isLive || !prefix.isEmpty())
    {
        ...
        // Логируем created/aborted по каждой попытке — это показывает:
        //   created=Y              => открылось, идём дальше
        //   created=N, aborted=N   => edge реально отклонил (сеть/403/timeout)
        //   created=N, aborted=Y   => ioCtrl отравлен внутренним abort demuxer'а
        bool created = false;
                ...
                if (muxer.isOk())
                {
                    ...
                    while (!demuxer.isAborted())
                    {
                        ...
                        if (!muxer.write(packet, idx))
                        {
                            ...
                            // Don't break — skip bad packets, continue recording
